...
\begin{abstract}
\emph{Walter} (\textbf{W}alking \textbf{A}lternating \textbf{T}ripod for \textbf{E}volutionary \textbf{R}esearch) is a twelve-degree-of-freedom hexapod built for this Engineering~90 capstone. The team fabricated a \textbf{complete hardware platform}: CAD-designed, 3D-printed body, legs, and linkages; hobby servos; Teensy~4.1 and Pololu eighteen-channel driver; SparkFun BMI270 IMU; power regulation; and three iterations of custom \emph{Hexapod V2} PCBs. On the control side, the project \textbf{completed two development tracks}: (i)~open-loop gait optimization in a reduced-order model with deterministic export to firmware (\texttt{hexapod\_openloop\_sine}), and (ii)~a MuJoCo~\cite{todorov2012mujoco} PPO~\cite{schulman2017proximal} training and evaluation pipeline (Stable-Baselines3~\cite{raffin2021stable}) with stored checkpoints, configs, and short evaluation summaries for representative locomotion experiments. Sec.~\ref{sec:results} summarizes surrogate metrics, exported gait parameters, a MuJoCo model view, and sample PPO evaluation means. The team \textbf{did not} implement the full layered adaptive firmware described in internal phase requirements (IMU-closed-loop reflexes and online adaptation on the MCU); that stack remains \textbf{design groundwork} for future work. Overall, the capstone establishes a \textbf{reproducible hexapod platform} and defensible optimization-to-firmware and simulation-based RL workflows; materials are at \url{https://github.com/HowardWHSrun/WALTER}. \textbf{Purchased cost:} electronics (three BOMs) US\$595.20; three PCB orders US\$197.47; \textbf{grand total US\$792.67} (one PCB order awaited payment at writing). Filament and fasteners are outside this sum.
\end{abstract}

\section{Introduction}

Reliable legged locomotion requires coordinating periodic limb motion with feedback under limited sensing and computation. Insects organize locomotor control in \emph{layers} that are standard in the literature: rhythmic coordination (often modeled as CPG-like structure), fast sensory feedback, and slower parameter adjustment~\cite{aydin2023mechanosensory,chen2021adaptive}. This capstone uses that organization as \textbf{bio-inspired control motivation}, not as a claim that every layer is already running on hardware.

\textbf{Framing of contributions.} The strongest completed work is \textbf{platform and methods}: a reproducible hexapod with CAD and custom PCBs; an \textbf{open-loop} gait optimization loop that exports parameters to firmware; and a \textbf{MuJoCo PPO} training and evaluation workflow with stored checkpoints and configs. A \textbf{layered adaptive} architecture (IMU fusion, reflexes, slow adaptation) is \textbf{specified in design documents} and informs observation choices in simulation, but it is \textbf{not fully implemented} on the MCU. The report therefore emphasizes what was built and verified, and treats full adaptive locomotion on the robot as \textbf{future engineering}, not an accomplishment of this submission.

\textbf{Bio-inspired elements (precise).} The \textbf{alternating tripod} is a biologically motivated low-dimensional gait prior: two stance groups alternate before any learning is applied. \textbf{IMU-based body-state cues} (roll, pitch, rates) match how many legged systems use gravito-inertial information for posture~\cite{valenti2015quaternion,bai2019posture} and align the simulation observation stack with how \emph{Walter} is instrumented, which matters for eventual sim-to-real. \textbf{Reinforcement learning in MuJoCo} explores dynamics and reward shaping under that prior; it does not, by itself, close the hardware adaptive loop. \textbf{Surrogate optimization} of a compact sinusoidal law isolates an interpretable rhythm before sensor-heavy layers are added---consistent with separating pattern generation from feedback in many biological models~\cite{chen2021adaptive}, but realized here only through open-loop firmware.

\textbf{Platform.} The physical system is \emph{Walter}, a twelve-DOF hexapod with 3D-printed structure, servos, Teensy, driver, IMU, and \emph{Hexapod V2} PCBs (Sec.~\ref{sec:mech}--\ref{sec:elec}).

\textbf{Code and report organization.} Source code and this document are maintained at \url{https://github.com/HowardWHSrun/WALTER}. Sec.~\ref{sec:math-bg} states the optimization objective and the PPO machinery used in software. Sec.~\ref{sec:track1}--\ref{sec:track3} and Table~\ref{tab:tracks} summarize three tracks: \textbf{(1)}~surrogate optimization to firmware (\textbf{completed}); \textbf{(2)}~PPO locomotion in simulation, with IMU-capable observation modes where configured (\textbf{completed} as a training/evaluation stack; limited hardware deployment); \textbf{(3)}~vision-augmented distance objectives (\textbf{future work}).
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.14\linewidth}p{0.46\linewidth}p{0.32\linewidth}@{}}
    ...
    III & Visual or pose features $f_t$; distance-centric rewards & \textbf{Not started:} specification and literature basis only \\
    \bottomrule
  \end{tabular}
\end{table}

...
\textbf{Deliverables.} \texttt{hexapod\_no\_imu\_optimization.py} (L-BFGS-B, constraints, random restarts); \texttt{no\_imu\_optimization\_summary.json}; \texttt{sync\_gait\_params\_from\_json.py}; \texttt{hexapod\_openloop\_sine.ino} with \texttt{optimized\_gait\_params.h}. Surrogate metrics and $\boldsymbol{\phi}^\star$ appear in Tables~\ref{tab:sim}--\ref{tab:opt-params}; the MuJoCo training model is illustrated in Fig.~\ref{fig:mujoco}.
...
Table~\ref{tab:opt-params} lists the corresponding optimized parameter vector $\boldsymbol{\phi}^\star$ (rest angles, phase shifts, frequency, and swing lift) exported to \texttt{optimized\_gait\_params.h}. Flexion phase shifts sit at the lower bound ($-90^\circ$), trading stride phasing for the surrogate's forward-speed and regularization terms; hardware tuning may move off that bound.
